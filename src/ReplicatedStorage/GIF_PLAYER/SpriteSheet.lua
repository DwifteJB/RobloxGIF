...
local SpriteSheetToUpdate = {}

function SpriteSheet:New(ImageLabel:ImageLabel, ImageWidth:number, ImageHeight:number, Rows:number, Columns:number, NumberOfFrames:number, Framerate:number, options)
	...
	table.insert(SpriteSheetToUpdate,self)

	return self

end

function SpriteSheet:Configure()
	...
	self.Offsets = {}
	for _ = 1,self.NumberOfFrames do
...
function SpriteSheet:Destroy()
	table.remove(SpriteSheetToUpdate,table.find(SpriteSheetToUpdate,self))
...
RunService.Heartbeat:Connect(function()
	for _, Frame in SpriteSheetToUpdate do
		if not Frame or not Frame.ImageLabel or not Frame.ImageLabel:IsDescendantOf(game) then
			table.remove(SpriteSheetToUpdate,table.find(SpriteSheetToUpdate,Frame))
		else
			Frame:Update()
		end
	end
